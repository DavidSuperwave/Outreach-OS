...
|---|---|---|---|---|---|---|
| `/login` `/signup` | ledger PublicApi chrome; kernel `passwordHash.ts` argon2id | `LoginPane` + hydrate `/api`; signed-in lands on `/` | shell 18; kernel-password unit; `POST_AUTH_PATH` | Chromium `/signup` → `/` | Kernel stock screens are transitional (OD-11). No Macro trade dress. | partial |
| Home bound | ADR-001; bound Playbooks + ICP → inspect → ask → table → Instantly reads | `HomePane` `data-surface=home.bound`; `home.focus-chat-input` | shell SSR surfaces; enter focuses ask | wrangler Chromium after login | Outreach-bound chrome, not extracted Neuwave home. Instantly reads only. | partial |
...
| Subscribe reconnect | WP-040 live update/reconnect | `/subscribe` replay from cursor; hydrate mints a short-lived, single-use ticket through `TaskSessionApi` before each `attachTaskSubscribe` open | workers Keep v2; unit drop → replayFrom → fresh-ticket reopen; ticket replay = 401 | n/a | Browser WebSockets cannot set Authorization; the long-lived kernel token never enters the URL. | in-slice |
