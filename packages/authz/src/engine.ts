 ...
 * tombstone / wrong tenant), then per-type policy code.
 */
export class PolicyEngine {
  ...
  mint<L extends AccessLevel, T extends EntityType>(request: MintRequest<T> & { need: L }): Receipt<L, T> {
    const { actor, entityId, need, state } = request;
    ...
    if (record.tenantId !== actor.actor.tenantId && actor.actor.tenantId !== null) {
      throw new AuthzError("cross_tenant", "actor tenant does not match entity tenant");
    }
    const have = POLICIES[accessType](state, actor.actor.id);
    if (!have || !levelSatisfies(have, need)) {
      throw new AuthzError("denied", `${actor.actor.id} lacks ${need} on ${entityId}`);
    }
    return mintReceipt({
      level: have as L,
...
export function requireReceipt<L extends AccessLevel, T extends EntityType>(
  receipt: Receipt<L, T>,
  need: L,
  entityId: string,
): void {
